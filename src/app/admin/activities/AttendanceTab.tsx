...
export default function AttendanceTab({ classes, schoolYear, user }: AttendanceTabProps) {
  const dropdown = useDropdown()
  const { invalidateAttendanceStudents } = useInvalidateQueries()
  ...
  // React Query hooks
  const { data: students = [], isLoading: loading } = useAttendanceStudents(selectedClassId, selectedDate, classes)
  const { data: currentDateHoliday } = useHolidayCheck(schoolYear?.id, selectedDate)

  // Helper to refetch students
  const fetchStudents = () => {
    if (selectedClassId) {
  ...
  const getThursdayOfWeek = (dateString: string): string => {
    ...
    if (dayIndex === 0) daysToThursday = -3
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative flex-1" data-dropdown>
            ...
            >
              ...
              <svg
                className={`w-[9px] h-[18px] text-black dark:text-white transition-transform ${dropdown.isOpen('class') ? 'rotate-180' : ''}`}
                viewBox="0 0 9 18"
                fill="none"
              >
                <path
                    ...
                    >
                      ...
                      <div className="w-[64px] flex items-center justify-center">
                        <div className="w-12 h-12 rounded-full bg-[#f5eaf6] flex items-center justify-center overflow-hidden">
                          {student.avatar_url ? (
                            ...
                            <span className="text-base font-medium text-[#8B8685]">{student.full_name.charAt(0)}</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="w-[150px]">
                        <span className={`text-base font-medium ${
                          student.has_thursday_attendance ? 'text-[#00a86b]' : student.has_compensatory_attendance ? 'text-[#00a86b]' : 'text-brand'
                        }`}>
                          {student.has_thursday_attendance ? 'Đã đi Thứ 5' : student.has_compensatory_attendance ? 'Đã bổ sung' : 'Chưa điểm danh'}
                        </span>
                      </div>
                      ...
                      <div className="w-[120px]">
                        {student.has_compensatory_attendance && !student.attendance_status ? (
                          ...
                          <div className={`h-[34px] px-4 rounded-full flex items-center gap-2 ${
                            student.attendance_status === 'present' ? 'bg-[rgba(0,168,107,0.12)]' : 'bg-[rgba(250,134,94,0.15)]'
                          }`}>
                            <div className={`w-[18px] h-[18px] rounded-full border-[1.5px] flex items-center justify-center ${
                              student.attendance_status === 'present' ? 'border-[#00a86b]' : 'border-brand'
                            }`}>
                              <Check className={`w-3 h-3 ${student.attendance_status === 'present' ? 'text-[#00a86b]' : 'text-brand'}`} strokeWidth={3} />
                            </div>
                            <span className={`text-sm font-medium ${student.attendance_status === 'present' ? 'text-[#00a86b]' : 'text-brand'}`}>
                      ...
                      <div className="flex-1 flex flex-col pl-5">
                        {student.has_compensatory_attendance && !student.attendance_status ? (
                          <>
                            ...
                            <span className="text-sm text-[#666d80] mt-0.5">Đã bổ sung cho Thứ 5 này</span>
                            <span className="text-sm text-[#666d80] mt-0.5">Điểm danh bởi: {student.compensatory_by || 'Không rõ'}</span>
                          </>
                        ) : student.attendance_status !== null ? (
                          <>
                            <span className="text-sm font-medium text-black dark:text-white leading-tight">
                            ...
                            <span className="text-sm text-[#666d80] mt-1">Điểm danh bởi: {student.attendance_by || 'Hệ thống'}</span>
                          </>
                        ) : null}
                      </div>
